...
use super::models::{
    KbChunk, KbCognition, KbDocDetail, KbFile, KbFileInput, KbGlobHit, KbHit, KbMemory, KbQuery,
    KbSettings, KbSettingsInput, KbStatus, MemoryApplyResult, MemoryCandidate, MODE_KEYWORD,
};
use super::worker::{self, KbHub};
use super::{chunk, embed, files, index, memory, search, settings};
...
pub fn kb_read(
    ...
    worker::drain_before_query(&app, REFRESH_BUDGET)?;
    let conn = state.db.lock().unwrap();
    read_detail(&conn, doc_id, &level.unwrap_or_else(|| "l1".into()), offset, limit)
}
...
pub fn read_detail(
    ...
    let full_chunks: Option<Vec<String>> = if source_type == "note" {
        source_id
            ...
            .and_then(|fid| {
                conn.query_row(
                    "SELECT content FROM kb_files WHERE id = ?1",
                    [fid],
                    |r| r.get::<_, String>(0),
                )
                .optional()
    ...
    let (total_chunks, chunks) = match &full_chunks {
        Some(all) => {
            let offset = if level == "l1" { 0 } else { offset.unwrap_or(0).max(0) };
            let limit = if level == "l1" { 1 } else { limit.unwrap_or(8).clamp(1, 64) };
...
#[tauri::command]
pub fn kb_memories(state: State<AppState>, mem_type: Option<String>) -> Result<Vec<KbMemory>> {
    let conn = state.db.lock().unwrap();
    memory::list(&conn, mem_type.as_deref())
}
...
mod tests {
    ...
    fn note_read_pages_through_full_source_beyond_cache_cap() {
        ...
        // 缓存确实被截断了（这正是要绕开的坑）
        let cached: i64 =
            conn.query_row("SELECT LENGTH(body) FROM kb_docs WHERE id = ?1", [doc_id], |r| r.get(0))
                .unwrap();
        assert!(cached < content.chars().count() as i64, "缓存应有截断上限");

        let first = read_detail(&conn, doc_id, "l2", Some(0), Some(8)).unwrap();
        assert!(first.total_chunks > cached as i64 / 300, "总块数应按真源算，而非缓存");
    ...
    fn note_l1_returns_first_chunk_and_full_total() {
        ...
        let expected = chunk::chunk_text(&content, chunk::DEFAULT_MAX_CHARS, chunk::DEFAULT_OVERLAP).len();
    ...
    fn derived_doc_read_unchanged() {
        ...
        .unwrap();
        let mem_id: i64 =
            conn.query_row("SELECT id FROM kb_memories LIMIT 1", [], |r| r.get(0)).unwrap();
