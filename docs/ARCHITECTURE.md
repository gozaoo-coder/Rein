...
| `resources/exercises.json` | 动作库种子（0025）：内置动作的唯一来源，由 `scripts/gen-exercises.mjs` 生成 —— 手写 SPEC（处方、要点、别名为手写；**肌群表必须显式给 `m`**，不再按名推导）+ `scripts/catalog/dataset.mjs`（exercises-dataset 精选子集，见 `resources/exercises.SOURCE.md`），禁止手改 | 运行时可变数据（用户自建动作落库，不回写种子） |
...
1. **课程条目只存处方**：`workout_plans.exercises_json` 的每一项 = `{exerciseId, 处方…}`。写入路径（`upsert_workout_plan` / 课程种子覆盖与合并 / `session_finish`）统一经 `exercise_lib::resolve` 解析：缺 `exerciseId` 的按名（含别名）挂库，库里没有就**自动建成自建动作**——所以任何历史自由命名都不会丢，也不会出现「没有库 id 的动作」。自动建库时**课程条目上已标的肌群会清洗后带进库记录**（此前写死 `{}`，导致同一动作在课程里有肌群图、在动作库里空白）。
...
**肌群模型（39 键）**：`config/muscles.ts` 的 `MUSCLE_KEYS` = 26 个浅/深层分区 + 13 个深层结构与补充肌束（大圆肌 / 菱形肌 / 肩袖 / 前锯肌 / 髂腰肌 / 臀小肌 / 股中间肌 / 肩胛提肌 / 胫骨后肌 / 腓骨肌群 / 跖肌 / 腘肌 / 股方肌）。三视图 SVG（BodyParts3D 投影）里这 39 个分区**都已画好**：浅层直接可见，深层默认折叠、激活时以 `.overlay` 再叠画一遍（否则会被浅层盖住，出现「标了看不见」）。`MUSCLE_LABELS` / `MUSCLE_DESCS` / `MUSCLE_GROUPS`（表单分组）/ `MUSCLE_VOLUME_TARGETS`（容量地标，`Record<MuscleKey>` 编译期查全）四处必须同步；`normalizeActivation()` 是**全端唯一的肌群校验入口**（白名单 + 1~3 档），Rust 侧 `exercise_lib::muscles::sanitize_muscles` 与 mock 同语义，漂移由 `seed.rs` 的单测（对比种子里的 `muscleKeys`）把守。

**动作要领（`steps`）**：内置动作可分步说明（`steps` JSON 数组，最多 12 步、单步 200 字），详情页渲染成编号列表；自建动作可在表单里逐行填写（导入来源见 `resources/exercises.SOURCE.md`）。清洗在 Rust `exercise_lib::steps` 与 mock 各一份等价实现。

**AI 动作库工具**：`list_exercises`（读，回**完整档位表**）、`upsert_exercise`（写自建动作，`muscles` 为 39 键枚举 + 1~3 档）、`set_exercise_muscles`（只改肌群，内置动作拒绝）。三个工具的 schema 共用 `ai/tools/muscleSchema.ts`，描述里写明档位语义与「先查库、与库内同名动作保持一致」。
...
- **超范围回弹分两类层**（`system/rubberScroll.ts`，模块头有曲线与逐 move 判定的完整推导）：**item 超伸** = 滚动框站住、只有内部内容层位移（`data-rubber-content`；文档页层 `data-rubber-page`），**容器自身超伸** = 滚动框整体位移（`data-rubber-self`）。**页面级滚动区一律走 item 型**（ModelsPage `.page`、KnowledgePage `.scroll`、AIPage `.msgs`、RunPage `.pane`、SessionOverlay `.scrollbody`、VoiceSessionView `.body`/`.tr`）：页面框与吸顶页头必须站住，动的只能是 item；自身型只留给横向条 / 纯文本盒这类「位移的就是它本身」的小容器。带定位 / 入场动画的元素不能当层（会与动画抢同一个 `transform`），包一层镜像布局的 `.rubber-layer` 承载位移。组件自己接管触摸时（抽屉拖高度、待办芯片），touchmove 必须挂**窗口捕获阶段**并 preventDefault：本模块在窗口冒泡上监听，冒泡里的拦截晚一步 —— 同一段位移会被两边各吃一份（抽屉曾因此出现「容器下移 offsetY + item 又叠 offsetY」的 2× 位移）。
- **运行时性能降级**：`system/perf.ts` 按 90 帧一个窗口统计掉帧（>32ms）占比，连续 2 个窗口 >30% 判定降级、连续 4 个窗口 <8% 恢复（恢复更保守，避免临界点抖动）；空闲时歇 6 秒不常驻 rAF，路由切换与流式输出用 `kickPerfWatch()` 插队。结论落 `<html data-perf="low">`，`base.css` 据此关掉 `backdrop-filter` 与循环动画（一次性过渡保留），并把 `--surface-translucent` 顶成实底。用户的 `auto|high|ultra|ultra-opt|low` 档位存 localStorage `rein.perf.v1`，UI 在「设置 › 性能」（档位清单 `PERF_MODES` 由设置页与画质预览页共用一份）。
- **超高档与液态玻璃**：`ultra` 在 `high` 之上再开 `components/common/GlassSurface.vue` 的折射表面（移植自 vue-bits 的 GlassSurface：SVG 位移贴图 + R/G/B 三通道分别位移再叠加，经 `backdrop-filter: url(#f)` 作用在元素自身，折射的是**它背后的真实内容**）。三道门缺一不可：用户选了超高档、没被降级、且内核认 `backdrop-filter: url()`（Safari / Firefox 会静默忽略整条声明，故有 `supportsSvgBackdrop()` 探测与「普通毛玻璃」退化分支）。`data-perf` 会写成 `ultra`（`auto|high|low` 的语义不变），实际管线另写 `data-glass`（`full|collapsed|off`）。回归：`scripts/e2e-perf-glass.mjs`（预览页逐档 + 真实 Dock：三块玻璃逐块折射 / 不参与布局 / 页签对比度真图采像素 + 参数调节面板）。
- **折射是有预算的：只给离散小控件**。`backdrop-filter: url(#f)` 每挂一块就是一次独立的背景滤镜，成本随**面积 × 每帧重算次数**涨，所以铺给谁不是"哪个更显眼"，而是"面积够不够小"。当前名单：底部 Dock 的三块玻璃、ToastHost 的提示条（它的 `fill` 单独给 `--surface` 的 86% 而不是 `--glass-fill` —— 提示条压在任意内容上、文字还是 nowrap 的一行短句，底越薄越过一句就看不清一句；这是 `fill` 这个口子存在的意义：同一套材质、不同浓度）。**大面积表面一律走 `.glass-surface` 的材质升级**：导航轨 / 信息栏 / 抽屉 / 会话面板不做折射（信息栏更明确 —— 它是一条可滚动的大面板，自己带 `backdrop-filter` 就会变成后代的 backdrop root，里面的问句条会退化成"只采到信息栏自己的内容"）；页头的圆钮用普通 `blur()`，因为 38px 的圆上折射带只有一两像素根本看不出来，还要压着页头那层渐进模糊（5 层 backdrop-filter）。
- **`ultra-opt`（超高（优化））是同一套观感的另一条链**（`glassPipeline`）：上游那条三通道链的三次 `feDisplacementMap`，scale 是 `distortionScale + 各通道 offset` —— 三个 offset 相等时三次位移逐像素相同，而三张单通道图 `screen` 复合正好还原原色（screen 在通道互斥时就是相加），整段复合是**恒等变换**。出厂值三个 offset 都是 0，所以塌缩成一次位移（10 个原语 → 3 个）与完整链逐像素等价（隔离台架实测 0 / 399900 像素差，430×260 面板 ×3 每帧 13.09ms → 1.69ms）。**恒等只在 offset 相等时成立** —— 调参面板把偏移拉开成色散后，`useCollapsed` 自己判定并退回完整链，任何档位下都不会画错。这条路上还顺手去掉了无效重烘：贴图指纹（尺寸 + 参数）没变就跳过、宽高取整、贴图里不掺实例 id（同尺寸的玻璃共用同一张 data URI，浏览器按 URL 缓存解码结果）、ResizeObserver 与参数监听合并到一帧一次、所有 SVG 属性写入先比旧值（实测写相同值与写变化值一样贵 —— 成本是「触发一次滤镜失效」而不是「写几个字节」）。
- **液态玻璃参数可调**（`system/glassParams.ts`）：管线上 12 个数字（边缘厚度 / 中心亮度 / 位移强度 / 三通道偏移 / 贴图分辨率除数 / 底色浓度…）摊在「画质预览 › 液态玻璃参数调节」面板里 —— 英文键名 + 中文名，滑杆给宽区间，数值点一下就地输入（吸附到步长、夹回区间）。GlassSurface 的默认值读这份（调用处仍只给尺寸与圆角），改动即时生效并落 localStorage `rein.glass.v1`；定稿 = 把选定的一组数写回 `GLASS_DEFAULTS`（面板的「恢复默认」就是回到它）。本地存的是**调参会话**（带 `rev`）：出厂值一换就把 `DEFAULTS_REV` +1，旧会话整份丢弃 —— 否则面板会一直显示上一组数，人对着旧值调还以为新出厂值没生效。
- **玻璃材质只有一份定义**（`styles/base.css` 的 `.glass-surface`）：六层叠出「一块有厚度的玻璃」—— ① 半透明底（`--glass-fill`）② 上亮下暗的受光描边（border-box 渐变，不占额外布局）③ 内顶高光（`--glass-sheen`）④ 外投影（`--glass-shadow`）⑤ 内圈细描边（`--glass-rim-2`）+ 上缘内晕（`--glass-caustic`）⑥ 外缘层（`--glass-halo`）。⑤⑥ 是**超高专属**：其余档位这两个令牌是透明的，所以每一档画的层数一致、只是观感不同（分层会让「标本」与真实界面各自漂移）。模糊与饱和度走 `--glass-blur` / `--glass-sat` 令牌。**四个悬浮条（运动 / 录音 / 语音 / 抢课）+ 导航轨 + 信息栏的问句条 + 课表吸顶表头**共用它；底部 Dock 与画质预览页的标本改用 `GlassSurface` 组件 —— 它要能整块折射，而折射必须是元素自己的 `backdrop-filter`。两族同源（同一套 `--glass-*` 令牌、同一组可调参数）。贴边表面加 `.glass-edge-*` 修饰（导航轨贴屏幕左缘 / 信息栏贴右缘 / 吸顶表头贴上缘，那三面不该画框）：受光边是**背景**画的，所以收掉其余三边的 `border-width` 不影响它。成本预算不同的表面就地覆盖 `--glass-blur`（课表表头是 8 个各挂一次 `backdrop-filter` 的格子，压到 14px）。内核完全不认 `backdrop-filter` 时由 `@supports` 退化成实底 + 内描边（不是"一层发灰的膜"）。
- **超高的观感差异来自光学层，不来自加 blur**：`backdrop-filter` 的模糊半径是这条链上单个最贵的旋钮（大面板上成本随半径线性涨），而加大它只是"更糊"。`html[data-perf='ultra']` 因此覆盖的是**整份 `--glass-*` 令牌**（底 0.52 → 0.42 更薄，加上 ⑤⑥ 与更深的外缘层），blur 与高画质档完全相同。覆盖令牌而不是逐个组件改样式，意味着所有用这套令牌的表面一起升级；弱档同理（`--glass-*` 被顶成实底，光学层自动归零 —— 它们只在 ultra 块里有非透明值）。回归把这条钉住：`scripts/e2e-perf-glass.mjs` 第 8 节逐档读令牌（高画质时 ⑤⑥ 透明、超高有值、弱档归零 + 模糊为 none），并断言两档的 blur 相同。
- **暗场材质上下文 `.stage-dark`**：压在深色内容上的玻璃（画质预览台的标本、参数面板的预览）在这一个类里换掉 fill / 受光边 / 内顶高光 / 活动底，前景因此永远是浅色（`--hero-text*`）、与当前主题无关 —— 暗底上的对比度不再"看运气"。两处原本各抄了一遍（"两份定义必然漂"那类事故的温床）。光学层刻意**不**在这里给值：它们是档位属性，标本要能与真实界面在同一档下逐条对上。弱档用 `html[data-perf='low'] .stage-dark` 再压一次 —— 通用弱档规则会把 fill 顶成 `--surface`（亮色下是白的），与这里的浅色前景会撞成白字白底。
- **Dock 是三块并列玻璃**（`layout/TabBar.vue`）：左独立圆钮（**用户自定义落点**，默认课表，长按换；`stores/dock.ts` / localStorage `rein.dock.v1`）+ 中药丸（内核两格 主页/我 + 插件条目，如运动）+ 右独立圆钮（AI，内核落点）。三块各是一块 `GlassSurface`，**并列留缝、互不叠压** —— 各自都带受光边，一叠就会在缝上出现一道月牙形硬边（与标本台同一条结论）。超高档三块**各自折射**背后的页面真实内容，底的浓度取 `--glass-fill`（**超高那一档的底比高画质更薄一档**：折射层后面就是页面本身，底厚了会把折射一起压平；对比度由台架守着，不是靠加厚底色）；nav 自己只是定位壳（不画底、不模糊）。其余档位与弱档 / 减弱透明度走退化分支。
- **Dock 的尺寸关系与前景只有一份定义**（`base.css` 的 `.dock-block` / `.dock-tab` / `.dock-slot`，含活动底的纯色与按压弹簧）：活动底是一块**中性纯色**（`--glass-tab-fill`，亮色浅白 / 暗色淡灰，值由主题决定），不是带走色相的渐变——压在玻璃上才不会被读成「盖了一块蓝色色板」，色相留给前景（`--accent-strong`）。**画质预览页的底栏标本直接用同一份** —— 标本的意义就是「所见即应用里的那一块」，两边各写一套必然漂（标本曾经用暗场令牌 + 自己的一版页签，与真实 Dock 长得不像，2026-09-24 修）。回归把「逐项一致」钉死：`scripts/e2e-perf-glass.mjs` 第 1 节逐项比对标本与真实 Dock 的计算样式与几何（块数 / 页签前景 / 活动底 / 玻璃底色 / 折射挂载），亮暗两色 + 弱档 + 减弱透明度各比一次；第 6 节另把「选中页签文字压在活动底上 ≥ 4.5:1」按真图采像素锁住（暗色是活动底最吃紧的一档）。
...
- **动效丰富程度是独立的一档**（`system/motion.ts`）：`off | default | rich`，与画质档位**正交** —— 画质回答的是「这块玻璃画不画得出来」，动效回答的是「界面要不要动」。一台撑得住超高折射的机器，用户也可能就是不想要动画；反过来，只能跑「流畅优先」的机器，选「关闭」也不该被理解成"降级"。**`default` 就是本文件写下的这一套**（不加任何装饰），`rich` 才启用液态玻璃的动作，`off` 关掉全部补间。生效档位是**算出来的**、只有一个写入点：用户档位 → 系统 `prefers-reduced-motion` 覆盖为 `off` → 掉帧降级压回 `default`（`rich` 那几样都是每帧合成，正在掉帧时再叠上去是火上浇油）；结论写 `<html data-motion>` 供 CSS 读，JS 侧的动效调用点（`rubberScroll` / `useDragDock` / `stores/ai` / `SessionOverlay` / `ProcessSection`）一律读 `motionOn` / `motionRich`，不再各自 `matchMedia`。「关闭」的语义是**只关补间、不关状态**：`:active` 的颜色与缩放终值照常生效，只是瞬时到位 —— 与 `prefers-reduced-motion` 那块同一条做法，也避开了弱档注释里那句「没有按压反馈会显得失灵」。档位存 localStorage `rein.motion.v1`，UI 在「设置 › 动效」与「画质预览 › 动效」各一份（共用 `MOTION_LEVELS`）。回归：`scripts/e2e-perf-glass.mjs` 第 7 节（三档各跑一遍：补间是否被压平 / 液态活动底的有无 / 纯色活动底的让位与回归）。
- **液态活动底**（`common/GlassThumb.vue`，丰富档）：等分容器里跟随索引平移的一颗 blob。三处关键选择：① **融合是涌现的** —— `feGaussianBlur` + `feColorMatrix` 的 alpha 对比把两颗球之间的 alpha 斜坡推成一道实心的颈，不手写颈部几何（也就不会有"脖子长歪"那类 bug）；② 位移走独立属性 `translate`、形变走 `scale` 的 keyframe，两个属性互不覆写，位移途中形变不会被打断（"加速拉宽压扁、减速收窄拉高"）；③ 浓度挂在**容器**上而不是 blob 上 —— goo 的 alpha 对比会把半透明输入推成实色（`19 × 0.62 − 8` 已经超出 1），挂在 blob 上会让「0.62 的活动底」在丰富档变成纯白，比默认档还亮一档。⚠️ 它带 `filter`，而带 `filter`/`mask`/`opacity<1` 的元素会成为后代的 backdrop root，**必须放在 `GlassSurface` 的 slot 里面**（`ProgressiveBlur.vue` 的文件头记着这个坑的三种变体）。挂载点：Dock 药丸、`SegmentedControl`（那里的 blob 换成「表面」语义 —— 分段控件坐在**不透明**轨道上，滑块要的是一块实色底，与压在玻璃上那层半透明 blob 不是一回事）、画质预览页的标本（标本不另画一遍，否则「所见即应用里的那一块」这条就断了）。gated 在 `html[data-motion='rich']`：`.goo` 容器里的纯色活动底让位给 blob。
- **按压定向光晕**（`composables/usePressGlow.ts`，丰富档）：把触点位置写进 `--gx` / `--gy`，**显隐交给 `:active`** —— 苹果对这条反馈的要求是"即时"，而 `:active` 是 pointer-down 当帧生效；让 JS 管开关就会多一段"等一个事件回来才亮"的延迟。一份监听挂在容器上、按 selector 就近匹配被点亮的那一格（事件总数与元素数无关），坐标落在**那一格自己**身上（`background-position` 相对元素自己算，写在容器上会让每一格的光晕都跑到左边）；按住横向拖时滑出哪一格就散掉，不跨格抢。两种挂法：`.press-glow`（背景只是一层纯色，光晕再叠一层 `background-image`）与 `.glow-layer`（背景另有用途 —— 活动底、轨道色 —— 走 `::after`）。页头圆钮走 `background-image`：它们的 `::after` 已经被 44×44 命中区占掉了。
- **透镜式进出**（丰富档）：抽屉 / 操作面板 / 菜单的遮罩从"一层黑影"改成"一层失焦" —— `backdrop-filter` 的模糊半径在 Chromium 里**可插值**，所以是渐变而不是硬切（`mask-image` 那类离散属性做不到）；`AppMenu` 的 bind 面板改成从**贴住的那个角**扩一个圆（`clip-path` 同形状之间插值，与 `transformOrigin` 用同一个判定，不必再量一遍像素）——缩放会把面板里的文字一起压扁，圆形揭示只换"看得见多少"，内容从头到尾都是清的。全视口遮罩模糊是丰富档**唯一**一处大面积新增合成，所以只给 6px、只在遮罩存在期间挂着，掉帧时由生效档位压回默认。
- **滚动边缘**（丰富档 + `composables/useScrolled.ts` 的 `useScrollCollapsed`）：向下滚时页头缩一档、把注意力让给内容，向上滚立刻还原 —— 判定的是**方向**不是位置（位置只能表达"滚了多远"，表达不了"用户此刻是想看更多内容还是想找回导航"）；到顶一律还原。只动 `scale`（独立属性，与圆钮的 `:active { transform: scale(.92) }` 并存），不动字号与内边距：滚动中改布局属性 = 整页重排 = 掉帧。超高档另在页头遮罩**之下**垫一层 `--bg` 渐变（苹果的「硬边缘」变体，纯渐变零合成成本），标题的落点更明确。
...
| `/settings/perf` | settings-perf | 画质预览（三级页：超高档的液态玻璃长什么样 —— 仅图标按钮 / 图标+文字按钮 / 底部栏（圆 + 药丸 + 圆，并列留缝；**与应用里的 Dock 共用同一份样式**，逐项一致）；可就地切档看折射与退化的差别；**动效档位也在这里就地切**（关闭 / 默认 / 丰富，台上标本跟着变 —— 丰富档下点页签能看见活动底的融合与形变，另有「试一下」按钮让底栏走一格）；台下入口拉开**液态玻璃参数调节面板**（12 项可调参数 + 自带暗场预览）；入口在「设置 › 性能 › 液态玻璃预览」） |
...
- **真机联调**：`cargo test --lib campus::course_select::tests::live -- --ignored --nocapture`（需 `REIN_GUET_USER` / `REIN_GUET_PASS`），或者**只读**的 `node scripts/probe-course-select.mjs`（账号从 App 库读、不需要环境变量，只打查询类接口，从不提交）。
- **2026-09-21 窗口开放后已核实**（批次 `1921`「2026-2027学年第一学期新生选课」，窗口 `09-21 10:00 ~ 09-30 23:00`）：EAMS 会话 → SSO 令牌 → 服务器时间 → 学生档案 → `open-turns` → `turn/{id}/select` → `query-condition` → `query-lesson`（395 行、一次给全，`pageInfo: null`）→ `simplest-lessons`（1013 行，**顶层就是数组**，不是 `{lessons}`）→ `selected-lessons`（14 行，含置课）。三处与代码假设不同、已按实测修正：**批次 id 在顶层 `turn.id`**（旧写法找的 `options.turn.id` 不存在，只是恰好与列表 id 相等才蒙对）；**`turnMode.showCount: false` 时 `stdCount` 一行都不返回**（「满没满」判不出来，只能靠提交结果说话）；我们发的 `sortField` / `sortType` 被忽略（响应 `sort__: {field: null, type: null}`，要排序得按 SPA 的真实参数名来）。
- **仍未验证**：`add-request` / `add-predicate` 的真实响应结构 —— **没有真提交过**。窗口里每一次提交都是真实的占位/选课，不能拿它当联调靶子；要验它得在明确同意下提交一门真实课程，或者等第一次真实抢课留下回执。
