 ...
 * `resources/workout_plans.json` 里该动作第一次出现的位置；肌群激活表取自
 * `src/config/muscles.ts` 的 `resolveActivation`（同一套关键词规则，生成后
 * 落成显式数据，库内不再跑正则）。
 *
 * 手写部分只有 SPEC 表：id / 分类 / 器材 / 别名 / 重量步进 / 少量补充动作的
 * 处方与要点。新增一个内置动作 = 在 SPEC 加一行并重跑本脚本。
 *
...
import { resolveActivation } from '../src/config/muscles.ts'
...
const SPEC = [
  // ---- 推 · 胸 / 肩 / 三头 ----
  { n: '杠铃卧推', id: 'barbell-bench-press', c: 'push', eq: 'barbell', a: ['卧推', '平板卧推'] },
  ...
  { n: '下斜俯卧撑（脚垫高）', id: 'decline-push-up', c: 'push', eq: 'bodyweight', a: ['下斜俯卧撑'] },
  { n: '椅式臂屈伸', id: 'bench-dip', c: 'push', eq: 'bodyweight', a: ['凳上臂屈伸'] },
  // ---- 拉 · 背 / 二头 / 前臂 / 后束 ----
  ...
  { n: '俯身反向飞鸟', id: 'reverse-fly', c: 'pull', eq: 'dumbbell', a: ['反向飞鸟'] },
  { n: '杠铃弯举', id: 'barbell-curl', c: 'pull', eq: 'barbell', a: ['二头弯举'] },
  { n: '锤式弯举', id: 'hammer-curl', c: 'pull', eq: 'dumbbell' },
  { n: '腕弯举', id: 'wrist-curl', c: 'pull', eq: 'dumbbell' },
  { n: '反握腕弯举', id: 'reverse-wrist-curl', c: 'pull', eq: 'dumbbell' },
  { n: '桌沿反向划船', id: 'table-inverted-row', c: 'pull', eq: 'bodyweight', a: ['反向划船'] },
  // ---- 腿 · 后链 / 小腿 ----
  { n: '杠铃深蹲', id: 'barbell-squat', c: 'legs', eq: 'barbell', a: ['深蹲', '背蹲'] },
  { n: '罗马尼亚硬拉', id: 'romanian-deadlift', c: 'legs', eq: 'barbell', a: ['RDL'] },
  { n: '保加利亚分腿蹲', id: 'bulgarian-split-squat', c: 'legs', eq: 'dumbbell', a: ['保加利亚蹲'] },
  { n: '坐姿腿弯举', id: 'seated-leg-curl', c: 'legs', eq: 'machine', a: ['腿弯举'] },
  { n: '腿屈伸', id: 'leg-extension', c: 'legs', eq: 'machine', a: ['坐姿腿屈伸'] },
  { n: '站姿提踵', id: 'standing-calf-raise', c: 'legs', eq: 'machine', a: ['站姿提踵'] },
  { n: '坐姿提踵', id: 'seated-calf-raise', c: 'legs', eq: 'machine' },
  { n: '反向弓步', id: 'reverse-lunge', c: 'legs', eq: 'bodyweight', a: ['弓步蹲'] },
  ...
  {
    n: '侧平板支撑', id: 'side-plank', c: 'core', eq: 'bodyweight', kindOverride: 'timed',
    d: { sets: 3, sec: 30, rest: 40 },
    tips: '侧链稳定：肘在肩正下方，髋部顶高成一条直线，两侧各做同样组数。',
  },
  {
    n: '罗马椅挺身', id: 'back-extension', c: 'legs', eq: 'bodyweight', a: ['山羊挺身'],
    d: { sets: 3, reps: 12, rest: 60 },
    tips: '后链与竖脊肌：髋铰链起落，脊柱保持中立，顶端不要过度后仰。',
  },
  {
    n: '椭圆机', id: 'elliptical', c: 'cardio', eq: 'cardio', kindOverride: 'cardio',
  ...
  {
    n: '动感单车', id: 'spin-bike', c: 'cardio', eq: 'cardio', kindOverride: 'cardio',
    d: { sets: 1, min: 25, rest: 0 },
    tips: '坐垫调到髋高，踏频 80-100 rpm，膝盖始终对准脚尖方向。',
  },
  {
    n: '跳绳', id: 'jump-rope', c: 'cardio', eq: 'other', kindOverride: 'cardio',
    d: { sets: 5, min: 3, rest: 45 },
    tips: '前脚掌落地、手腕摇绳，落地轻盈；膝踝不适换成原地小跳。',
  },
  {
    n: '泡沫轴放松', id: 'foam-roll', c: 'mobility', eq: 'other', kindOverride: 'timed', m: {},
    d: { sets: 1, sec: 300, rest: 0 },
    tips: '每个酸痛点半分钟慢速滚动，不滚腰椎与关节，呼吸放松不憋气。',
  },
  {
    n: '静态拉伸', id: 'static-stretch', c: 'mobility', eq: 'other', kindOverride: 'timed', m: {},
...
function buildEntry(spec, plans) {
  ...
  // 显式 m（含 `{}` = 柔韧类不做肌群标注）优先；缺省走名称规则
  const muscles = 'm' in spec ? spec.m : resolveActivation(spec.n)
  if (!muscles) {
    throw new Error(`动作「${spec.n}」无法解析肌群：请在 SPEC 里显式给 m 字段`)
  }
...
function main() {
  ...
  const entries = SPEC.map((s) => buildEntry(s, plans))
  ...
  const out = { version: 1, exercises: entries }
  const file = path.join(root, 'resources', 'exercises.json')
  fs.writeFileSync(file, JSON.stringify(out, null, 2) + '\n', 'utf8')
  console.log(`已写入 ${path.relative(root, file)}：${entries.length} 个动作（覆盖课程种子 ${plans.size} 个动作）`)
}
